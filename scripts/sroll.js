...
window.addEventListener("scroll", () => {
  ...
  if (scrollY > 65) {
    NAV_BAR.classList.add("bg");
  } else {
    NAV_BAR.classList.remove("bg");
  }
});
...
function saveInput() {
  NAV_DIST.forEach((dist) => {
    ...
    if (TOP <= 0 && BOTTOM >= 0) {
      ...
      if (dist.getAttribute("id") == "characters") {
        changeActiveNav("characters");
      }
      if (dist.getAttribute("id") == "episodes") {
        changeActiveNav("episodes");
      }
    }
  });
}
